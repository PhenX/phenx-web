...
function initCanvas(canvas) {
  if (window.G_vmlCanvasManager && window.attachEvent && !window.opera) {
	...
	/*else if (window.safari3) {
		var p, 
		    ctx = window._CanvasRenderingContext2D,
		    ctxp = ctx.prototype,
		    canvasContext = canvas.getContext('2d');

		for(p in ctxp) {
      canvasContext[p] = ((typeof ctxp[p] == 'function') ? ctxp[p].bind(canvasContext) : ctxp[p]);
			//canvasContext[p] = ctxp[p];
    }
	}*/
	return canvas;
}
...
(function(){
  ...
  // what is the browser's implementation ?
	var moz = !!CanvasRenderingContext2D.prototype.mozDrawText && !CanvasRenderingContext2D.prototype.strokeText,
      whatwg = !!CanvasRenderingContext2D.prototype.strokeText;
